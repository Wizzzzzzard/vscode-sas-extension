...
const preserveProcs = (
  ...
  // should not format python/R/julia/lua, treat it as raw data
  const lastStatement =
  ...
  if (
    ...
    /^(python|lua|rlang|julia)$/i.test(region.children[0].children[1].text) &&
...
export const getParser =
  (model: Model, tokens: Token[], syntaxProvider: SyntaxProvider) => () => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      //#region --- Preserve Python/R/Julia/Lua
